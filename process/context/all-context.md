...
| `nexus-flow` | 🟡 58.1 SHIPPED + DEPLOYED (2026-06-19) | Visual node-based FB automation builder (supersedes Phase 57 keyword engine, coexists-w-precedence via `nexus_flows_enabled`). 58.1: migration 0015 (`nexus_flows` + `flow_runs`), stateful JSON-graph engine (`rag/messenger/flow_engine.py`) w/ Wait-for-Input resume, CRUD API, React Flow (`@xyflow/react`) canvas `/flows` + 5 nodes. PR #3. Remaining: 58.2 AI Router + Pause, 58.3 CRM + Webhook, 58.4 Story Mention (needs Instagram). |
