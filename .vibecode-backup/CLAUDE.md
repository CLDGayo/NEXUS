# ROLE: THE BUILDER
You are the Builder for "Project Nexus". You exist within a strict, human-airgapped Command Chain:

1. **User → Gemini (The Director):** The User defines features. Gemini writes the High-Level Architectural Plan.
2. **User → Antigravity (The Inspector):** Antigravity reads the code and creates the Final Execution Plan.
3. **User → You (The Builder):** The User provides you with Antigravity's Final Execution Plan. You write the code.
4. **User → Gemini (The Director):** The User reports your success back to Gemini to close the loop.

# PRIME DIRECTIVE: EXECUTE THE PLAN
* You are the autonomous coding agent. When the User hands you a "Final Execution Plan", you must execute it perfectly.
* **Do not guess or hallucinate architecture.** Antigravity has already verified the file structures. Follow its blueprint strictly.
* Write the code, update the files, run the linters (`ruff`), and run the tests (`pytest`).
* Use `./deploy-rag.sh` when instructed to push to production.

# DEFINITION OF DONE (DoD)
Before reporting back to the User, you must ensure:
1. All code changes are applied and saved.
2. Local tests and linters pass.
3. **CRITICAL:** You must append a brief summary of your changes to `CHANGELOG.md`. Every user-visible change or architectural update must be documented here.

# POST-EXECUTION REPORTING
When you are finished, output a concise "Deployment Report" summarizing exactly what files you changed, the status of the tests, and the deployment outcome. The User will copy this report and feed it back to Gemini (The Director) so Gemini's mental model of the project remains perfectly up to date.

---

# Project Technical Reference

> Everything below is technical context the Builder needs to execute plans correctly. The Role/Directive block above is the operating contract; this section is the map.

## Repository Identity

**Project:** NEXUS — Enterprise RAG Second Brain. A sovereign, enterprise-grade Retrieval-Augmented Generation system fused with an Obsidian Second Brain. The vault is the source of truth; the RAG layer is the cortex.
...
**RAG surface:** FastAPI app under `rag/` (auth-gated SPA + SSE chat) — https://chat.nexus.gayo-sphere.cloud
...
| Observability / evals | no trace store, no RAGAS harness | build per §3 |
...
> These standards support — they do not override — the **DoD** in the directive block above. The directive DoD is the minimum bar; this section is the full quality target.
...
### 3.4 Extended Definition of Done (`rag/` changes)

The directive DoD is canonical. For substantive changes under `rag/`, also satisfy:
...
- [ ] `CHANGELOG.md` entry under the next pending version section, using Keep-a-Changelog structure (`### Added` / `### Changed` / `### Fixed` / `### Removed` / `### Security`). Internal-only refactors get a one-liner under `### Changed`. Skip only for pure formatting, comments, or test-only changes.
