...
export default function RoadmapCard({ item }) {
  ...
  return (
    <div className="relative h-full overflow-hidden rounded-xl border border-nexus-border bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-slate-400">
          {Icon ? <Icon size={20} /> : null}
        </div>
        <span className="inline-flex items-center gap-1 rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-slate-600">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-white/30 backdrop-blur-sm">
        <span className="flex h-9 w-9 items-center justify-center rounded-full border border-nexus-border bg-white/90 text-slate-500 shadow-sm">
          <Lock size={15} />
        </span>
      </div>
    </div>
