...
async def _enrich(
    product_ids: list[uuid.UUID], tenant_slug: str
) -> list[Product]:
...
async def _image_url_for(image: ProductImage) -> str | None:
    ...
    if not image.storage_key:
        return None
    try:
        return await object_store.presigned_get_url(
        ...
        _log.warning(
            "product_branch.presigned_failed key=%s detail=%s",
            image.storage_key,
            exc,
        )
        return None
...
def _ctx_url_for(product: Product) -> str | None:
    ...
    try:
        return template.format(
            slug=product.slug, id=str(product.id)
        )
