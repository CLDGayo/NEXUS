...
### Added
- **feat(i18n): Phase 59 — Multi-language Chatbot & Tenant Language Preference.** Workspaces can set a default chatbot language; the NEXUS Flow engine injects a strict "reply exclusively in <language>" directive into LLM-node system prompts so the bot answers users in that language. Default `en` → byte-identical behavior for existing tenants.
  - `rag/i18n.py` — new shared, dependency-free module: `LANGUAGE_NAMES` (code→English name, mirrors `nexus-ui/src/i18n/languages.js`), `SUPPORTED_LANGUAGES` (derived, no drift), `language_directive(code)` (empty for `en`/`None`/unknown, else `"You must reply exclusively in <Name>."`), and `apply_language_directive(prompt, code)`. `rag/auth/schemas.py` now imports `SUPPORTED_LANGUAGES` from here (single source of truth for both user- and workspace-language validators).
  - `rag/migrations/versions/0018_phase59_tenant_language.py` — `ADD COLUMN preferred_language VARCHAR(8) NOT NULL DEFAULT 'en'` on `app.tenants` (mirrors the Phase 54 user-language migration). `rag/database/models.py` `Tenant` gains the matching mapped column.
  - `rag/auth/schemas.py` — `TenantRead.preferred_language` (default `"en"`) and `TenantUpdate.preferred_language` (optional, validated against `SUPPORTED_LANGUAGES`). All four `TenantRead` build sites (`v2_tenants.create_tenant` / `get_tenant_detail` / `_tenant_read_from_model`, `auth/tenant.list_tenants_for_user`) now pass it; `PATCH /api/tenants/{id}` accepts and persists it.
  - `rag/messenger/flow_engine.py` — `run_flow_job` and `resume_flow_for_dm` load `Tenant.preferred_language` and thread it into `_traverse(language=...)`; the `aiRouter` LLM node wraps its classifier system prompt with `apply_language_directive`. No-op for `en`.
  - `nexus-ui/src/components/workspace/GeneralTab.jsx` — Workspace Settings → General gains a frosted Radix **Chatbot language** `Select` (options from the shared `LANGUAGES` list) bound to `preferred_language`, included in the existing PATCH save path; read-only view for non-managers.
  - Tests: `rag/tests/test_phase59_tenant_language.py` (directive + validator units) and two `test_flow_engine.py` cases asserting a non-English tenant injects the Spanish directive into the aiRouter system prompt while an English tenant's prompt is unchanged.
